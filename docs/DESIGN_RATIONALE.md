...
## 35. Milestone 13a: scoping by consumer, and a claim the plan had been making for eleven milestones

Planning milestone 13 (Practice / reverse mode) started, as every milestone now does, with a short
audit of §9 against its neighbors. It found three things sitting in milestone 13 that had nothing
in milestone 13 to attach to, and one sentence the plan had been repeating since it was written
that turned out not to be true.

**Scoping by consumer, not by section.** D27 asks for 24 authored programs — 8 concepts × 3
levels. D33 bars reverse mode from the two algorithm concepts, on the grounds that reassembling
bubble sort from shuffled lines tests memory rather than understanding. Those two facts are both
locked, both correct, and together they mean 6 of the 24 programs have no consumer until
flowcharts exist in milestone 14. The same held for AC-9.12: D32 defines hint level as how many
_flowchart cards_ start pre-filled, so a hint selector in milestone 13 would be a control that
visibly changes nothing. Both moved to milestone 14, and — this is the part that matters — moved
into the **Build milestones table's own row 14**, not just into a note on the acceptance criteria.
The milestone table is the operative definition of what a milestone contains; a deferral recorded
anywhere else is a deferral that milestone 14 can legitimately not know about. The project's own
first audit found seven pieces of promised work that no milestone owned, which is precisely what
speaking a deferral instead of writing it produces.

The trade-off is stated rather than hidden: if D28 cuts flowcharts whole, AC-9.11 never fully
closes and AC-9.12 is never demonstrated. That is the right outcome — both items exist to serve
flowcharts — but it means v1 could ship with a criterion permanently annotated as partially met,
and that is better than authoring six programs and a dead selector so a checklist can be green.

**The claim that had been true in the plan and false in the code.** §9 says the flowchart is
"generated by parsing the program (§1 validator already parses it)," and, later, that "the diagram
is built by walking the same structure the validator already produces." Both sentences describe
milestone 14's central mechanism as free. Neither is true. `src/subset/parser.ts` is a
recursive-descent **recognizer**: every one of its methods returns `void`. `parseSuite()` — the one
place block nesting is genuinely understood — consumes INDENT, loops until DEDENT, and returns
nothing. The nesting is known for exactly as long as it takes to validate it, then discarded. What
survives a parse is a boolean and, on failure, one line number.

This changes nothing about milestone 13: D34 defines reverse-mode blocks as "the program's own
lines, shuffled," and the tokenizer already emits per-line structure with explicit INDENT/DEDENT
markers, which is more than enough. It changes milestone 14 substantially — the largest single item
in the section, already the designated candidate to be cut whole, turns out to need a parse tree
built before it can start. Finding that during milestone 13's planning rather than milestone 14's
first day is worth more than the correction itself, and it is a direct argument for the standing
pre-build audit (`decisions/003`) having been made permanent rather than left as a one-time pass
before milestone 1: the audit's whole value is that it re-checks assumptions that were true when
