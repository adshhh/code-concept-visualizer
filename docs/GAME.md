...
## Practice: reverse mode (13a, AC-9.11/9.13/9.14/9.15/9.16's foundation)

D34: "every exercise derives from one authored example program." The program is the only
hand-written artifact — the blocks, the shuffle, and the answer check all derive from it.
13a builds the corpus and that derivation/checking logic, with no new UI; 13b (route, drag +
keyboard reordering, divergence animation) is next.

### Scope: 18 programs now, 6 with flowcharts

D27 asks for 24 authored programs (8 concepts × 3 levels). D33 scopes reverse mode to the 6
basics only — reassembling an algorithm from shuffled lines tests memory, not
understanding — so binary search's and bubble sort's 6 programs have no consumer until
flowcharts (m14). AC-9.12 (hint level) moves with them: D32 defines hint level as how many
*flowchart cards* start pre-filled, and m13 has no cards. Both are recorded in the Build
milestones table's own row 14, not just in a note here — see
`docs/decisions/004-practice-scope-split.md`.

### A claim in the plan that wasn't true

§9 says the flowchart is "generated by parsing the program (§1 validator already parses it)."
`src/subset/parser.ts` is a recognizer: every method returns `void`, and `parseSuite()`
recognizes block nesting only for the instant it takes to consume it, then discards it. There
is no AST anywhere in the codebase. This doesn't block reverse mode — D34 only needs "the
program's own lines," and the tokenizer already emits per-line INDENT/DEDENT/NEWLINE
structure — but it means m14 has to build a real tree rather than inherit one. Corrected in
the plan and logged as its own finding in `DESIGN_RATIONALE.md` §35.

### Blocks and the answer check

`src/game/blocks.ts`: one block per non-blank source line, keeping its own indentation
(§9 locks this for v1 — the user orders blocks, not their indentation). `toBlocks`'s
precondition — one statement per physical line — is checked, not assumed:
`checkBlockSplittable` rejects a bracket-continued statement, a single-line suite
(`if True: print(1)`, which `tests/fixtures/accepted/28_single_line_suites.py` proves the
tokenizer accepts), and blank/comment lines, all of which would silently corrupt a
line-based split. Every one of the 18 programs is asserted against it.

`shuffleBlocks` is seeded on the program id, never `Math.random()` — a reload must not
change the puzzle — and retries with a bumped seed if it ever lands on the solution.

`src/game/reverseMode.ts`'s `checkAttempt` takes only `{ stdout, frames }` plus the expected
output — never the original program, since `src/architecture.test.ts` forbids `src/game/`
from importing `src/engine/`, and because D34's honest answer is "produces the expected
output," not "matches the original order." A different arrangement that still prints the
right thing is correct.

**The divergence step is exact, not approximate (AC-9.16).** `frame.stdout` is cumulative
output as of that step, so the first frame whose output stops being a prefix of the expected
output is the exact frame the arrangement went wrong — feedable straight to `usePlayback`'s
`goToStep`. Pinned against the real engine in `src/engine/reverseMode.test.ts` for four real
misarrangements: a hoisted `print` that runs to completion with the wrong output, a crash
before anything prints, a different-but-still-correct line order, and an indented-line-first
arrangement that never reaches the engine at all (the validator rejects it before any frame
exists).

### Measured, not assumed: how often can a wrong answer actually be animated?

Every recent milestone has found something by running code instead of reasoning about it;
this measurement continues that discipline (`src/practice/measureArrangements.test.ts`, run
via `MEASURE=1 npx vitest run src/practice/measureArrangements.test.ts`, since it's a
few-hundred-thousand-run measurement rather than a fast gate).

Two populations, because they answer different questions. The **whole arrangement space**
(exhaustive up to 6 blocks, 3000-sample above) asks "is any exercise trivially satisfiable" —
answer: no, every program has only 1–4 correct orderings out of 24 to 3000+ candidates. But a
learner dragging blocks produces **near-misses**, not uniform-random permutations, and that
population behaves completely differently:

| | whole space, animatable | near-miss (one block moved), animatable |
|---|---|---|
| range across all 18 programs | 0.1% – 20.8% | 1.8% – 35.0% |
| median | ~2.8% | ~16.7% |

The near-miss numbers are what actually matters for 13b's UX, and they say a learner who is
close but wrong will see a real animated run a meaningful fraction of the time, not a rare
one — validating an assumption the plan made but never checked.

**One real bug this measurement found, fixed in the corpus itself.** `if-else-medium`
measured at **0.0% animatable** for near-misses — every single one-block move was rejected
by the validator, because the program's entire body was one rigid nested `for`/`if`/`elif`/
`else` statement with no independent top-level line to move. No wrong answer could ever be
animated for that exercise; AC-9.16 would have been structurally unreachable for it, not
merely rare. Fixed by lifting the list literal onto its own top-level line, which brought it
to 1.8% — verifiably non-zero, not just plausible. `registry.test.ts` now asserts every
program has at least two top-level statements, so this class of bug can't return silently.

### Expected output: generated, not typed

`tests/fixtures/practice/expected-output.json` is a committed snapshot of what the real
engine prints for each of the 18 programs — same "a changed snapshot may never be silently
...
**13a:** `src/practice/programs/*.py` (new, 18) · `src/practice/types.ts`, `registry.ts` (new)
· `src/practice/registry.test.ts` (new — real-engine, `describe.each`) ·
`src/practice/measureArrangements.test.ts` (new — the measurement above, `MEASURE=1`-gated) ·
`src/game/blocks.ts` + `blocks.test.ts` (new) · `src/game/reverseMode.ts` +
